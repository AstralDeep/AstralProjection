"""Tests for astral_client/local_agents.py and win_agent/byo_host.py: derived agent
inventory including stopped bundles, the Agents dialog's list/refresh/stop wiring,
and Settings-menu hosting-profile gating.
"""
...
def test_inventory_is_derived_and_read_only(tmp_path):
    ...
    (tmp_path / "ua-broken-3.pending").mkdir()
    ...
    assert by_id["ua-sorter-2"] == {"agent_id": "ua-sorter-2", "name": "Inbox sorter",
                                    ...
                                    "revision": ""}
    assert [r["agent_id"] for r in rows][0] == "ua-greeter-1"
...
def test_inventory_skips_and_prunes_a_root_whose_revisions_are_gone(tmp_path):
    ...
    host._prune_empty_agent_root("ua-kept-8")
    host._prune_empty_agent_root("ua-legacy-7")
    assert kept.is_dir() and os.path.isdir(legacy)
    host._prune_empty_agent_root("ua-absent-0")
