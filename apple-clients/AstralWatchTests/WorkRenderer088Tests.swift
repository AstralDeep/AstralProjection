...
final class WorkRenderer088Tests: XCTestCase {
    ...
    func testReconnectRendersSelectedWorkUnavailableUntilExplicitRetry() async throws {
        let model = WatchModel()
        model.workVisible = true
        model.connected = true
        let loading = try png(WatchWorkSurfaceView().environment(model))
        await model.handle(.disconnected(reason: "synthetic"))
        let disconnected = try png(WatchWorkSurfaceView().environment(model))
        XCTAssertNotEqual(loading, disconnected)
        XCTAssertTrue(model.beginConversationConnection(generation))
        model.connected = true
        let reconnected = try png(WatchWorkSurfaceView().environment(model))
        XCTAssertNotEqual(loading, reconnected)
