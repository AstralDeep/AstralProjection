...
final class ViewportSnapshotTests: XCTestCase {
    ...
    private func model() -> (WatchModel, Frames) {
    ...
    private func resize(_ model: WatchModel, width: Int) {
        model.viewportChanged(width: width, height: 800, scale: 2, reducedMotion: false)
    }

    private func finish(_ model: WatchModel, _ frames: Frames, columns: Int = 1) {
    ...
    func testLatestViewportCoalescesAndSnapshotPreservesPresentation() async {
        let (model, frames) = model()
        model.pendingDictation = "Unsent words"
        ...
        XCTAssertEqual(model.pendingDictation, "Unsent words")
        XCTAssertEqual(model.entries.map(\.id), before.map(\.id))
