...
enum class LocalVoiceDisposition(val wireValue: String) {
    READY("ready"), TYPED_FALLBACK("typed_fallback"), REJECTED("rejected"),
    PERMISSION_DENIED("permission_denied"), FINAL("final"), SPEAKING("speaking"), FINISHED("finished"),
}
...
data class LocalVoiceCapability(val disposition: LocalVoiceDisposition, val payload: JsonObject) {
    companion object {
        private val fields = setOf("contract", "transport", "configured_locale", "full_duplex", "has_microphone", "has_audio_output", "microphone_permission", "recognition_permission", "recognition_processing", "recognition_locale", "recognition_installation", "synthesis_processing", "synthesis_locale")
        private val unavailable = setOf("schema_version", "speech_backend", "status", "reason", "checked_at", "expires_at", "supported_transports", "requirements")
        fun fromJson(value: JsonObject): LocalVoiceCapability? {
            if (value["status"] != null) {
                if (value.keys !in setOf(unavailable, unavailable + "retry_after_seconds") || value.string("schema_version") != "2" || value.string("speech_backend") != "client_local" || value.string("status") != "unavailable" || value.string("reason") !in LocalVoiceFrame.reasons || value.array("supported_transports")?.mapNotNull { it.jsonPrimitive.contentOrNull } != listOf("client_local") || value.obj("requirements") == null) return null
...
data class LocalVoiceFrame(val type: String, val disposition: LocalVoiceDisposition, val payload: JsonObject) {
    companion object {
        ...
        private val common = setOf("type", "schema_version", "speech_backend", "device_id", "connection_generation", "session_id", "generation", "speech_revision")
        ...
        fun fromJson(value: JsonObject): LocalVoiceFrame? {
            ...
            if (value.keys != fields[type] || value.string("schema_version") != "2" || value.string("speech_backend") != "client_local" || listOf("device_id", "connection_generation", "session_id").any { !isUuid4(value.string(it)) } || value.int("generation")?.takeIf { it > 0 } == null || value.int("speech_revision")?.takeIf { it > 0 } == null) return null
...
private fun isUuid4(value: String?): Boolean = value?.matches(Regex("^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$")) == true
